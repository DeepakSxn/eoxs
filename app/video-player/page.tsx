    ...
    useEffect(() => {
      ...
      const unsubscribe = auth.onAuthStateChanged(async (currentUser) => {
        if (currentUser) {
          ...
          // Fetch user data to get professions
          try {
            ...
            if (!querySnapshot.empty) {
              ...
              // Get user professions (primary and secondary if available)
              const professions: string[] = []

              if (userData.primaryProfession) {
                professions.push(userData.primaryProfession)
              } else if (userData.profession) {
                professions.push(userData.profession)
              }

              if (userData.secondaryProfession) {
                professions.push(userData.secondaryProfession)
              }
  ...
  try {
    ...
    const organizeIntoModules = (videos: Video[]) => {
      ...
      userProfessions.forEach((profession) => {
        if (videosByCategory[profession]) {
          ...
          delete videosByCategory[profession]
        }
      })

      // 3. Add other categories as modules (except Miscellaneous)
      ...
      if (videosByCategory["Miscellaneous"]) {
        moduleArray.push({
          name: "Miscellaneous",
    ...
    const playNextVideo = () => {
      ...
      if (nextIndex < playlist.videos.length) {
        ...
        if (videoWatchEvents[nextVideoId] === true || videoWatchEvents[nextVideoId] === false) {
          videoChangeRef.current = true
          setCurrentVideoIndex(nextIndex)
          setCurrentVideo(playlist.videos[nextIndex])
          setProgress(0)
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            disabled={
                              ...
                              !isVideoPlayable(currentVideoIndex + 1)
                            }
                          >
